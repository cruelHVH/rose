...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // ImRect clip(bb.Min + style.FramePadding, bb.Max - style.FramePadding);
    // draw->AddText(text_pos, IM_COL32(255, 255, 255, 255), label, label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float* animation_ptr = window->StateStorage.GetFloatRef(id, 0.0f);
    *animation_ptr = ImLerp(*animation_ptr, *v ? 1.0f : 0.0f, g.IO.DeltaTime * 10.0f);
    if (is_visible)
    {
        ImVec4 base_fill_color = ImVec4(30.0f / 255.0f, 30.0f / 255.0f, 30.0f / 255.0f, 1.0f);
        ImVec4 base_inline_color = ImVec4(60.0f / 255.0f, 60.0f / 255.0f, 60.0f / 255.0f, 1.0f);
        ImVec4 lerped_fill_color = ImLerp(base_fill_color, menu::accent_color, *animation_ptr);
        ImVec4 lerped_inline_color = ImLerp(base_inline_color, menu::accent_color, *animation_ptr);
        window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, IM_COL32(35, 35, 35, 255), IM_COL32(35, 35, 35, 255), IM_COL32(25, 25, 25, 255), IM_COL32(25, 25, 25, 255));
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ColorConvertFloat4ToU32(lerped_fill_color), 0.0f);
        window->DrawList->AddRect(ImVec2(check_bb.Min.x + 1, check_bb.Min.y + 1), ImVec2(check_bb.Max.x - 1, check_bb.Max.y - 1), ColorConvertFloat4ToU32(lerped_inline_color), 0.0f, 0, 1.0f);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, IM_COL32(0, 0, 0, 255), 0.0f, 0, 1.0f);
    }
    const ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 4.0f);
    if (g.LogEnabled)
    ...
    if (is_visible && label_size.x > 0.0f)
    {
        for (int x = -1; x <= 1; x++)
        {
            ...
            {
                if (x == 0 && y == 0) continue;
                window->DrawList->AddText(ImVec2(label_pos.x + x, label_pos.y + y), IM_COL32(0, 0, 0, 255), label);
            }
        }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const float width = 267.f;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float combo_height = 20.0f;

    const ImRect rect(pos, pos + ImVec2(width, label_size.y > 0 ? combo_height + label_size.y + style.ItemInnerSpacing.y : combo_height));
    ImRect clickable(rect.Min + ImVec2(0, label_size.y > 0 ? label_size.y + style.ItemInnerSpacing.y : 0), rect.Max);
    ...
    ImU32 inline_col_troll = IM_COL32(40, 40, 40, 255);

    ImGuiStorage* storage = &window->StateStorage;
    float tween_value = storage->GetFloat(ImHashStr("##tween", 0, id), 0.0f);

    float target = (hovered || popup_open) ? 1.0f : 0.0f;
    float speed = 8.0f * g.IO.DeltaTime;
    tween_value = ImLerp(tween_value, target, speed);
    storage->SetFloat(ImHashStr("##tween", 0, id), tween_value);

    ImVec4 accent_color = menu::accent_color;
    ImVec4 normal_color = ImVec4(0.f, 0.f, 0.f, 1.0f);
    ImVec4 lerped_color = ImLerp(normal_color, accent_color, tween_value);
    ImU32 stroke_col = GetColorU32(lerped_color);

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(2, 2), clickable.Max - ImVec2(2, 2), IM_COL32(30,30,30,255), style.FrameRounding, 0);

    window->DrawList->AddRect(clickable.Min, clickable.Max, stroke_col, style.FrameRounding, 0, 1.0f);
    window->DrawList->AddRect(clickable.Min + ImVec2(1, 1), clickable.Max - ImVec2(1, 1), inline_col_troll, style.FrameRounding, 0, 1.0f);

    const char* symbol = popup_open ? "-" : "+";
    ImVec2 symbol_pos = ImVec2(clickable.Max.x - 20.0f, clickable.Min.y + (clickable.GetHeight() - g.FontSize) * 0.5f);

    window->DrawList->AddText(symbol_pos + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255), symbol);
    window->DrawList->AddText(symbol_pos + ImVec2(1, 0), IM_COL32(0, 0, 0, 255), symbol);
    window->DrawList->AddText(symbol_pos + ImVec2(0, -1), IM_COL32(0, 0, 0, 255), symbol);
    window->DrawList->AddText(symbol_pos + ImVec2(0, 1), IM_COL32(0, 0, 0, 255), symbol);
    window->DrawList->AddText(symbol_pos, IM_COL32(255, 255, 255, 255), symbol);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        ImVec2 text_pos = clickable.Min + ImVec2(10.0f, (clickable.GetHeight() - g.FontSize) * 0.5f);
        ImVec2 text_clip_max = ImVec2(clickable.Max.x - 25.0f, clickable.Max.y);
        ImVec4 clip_rect = ImVec4(clickable.Min.x, clickable.Min.y, text_clip_max.x, text_clip_max.y);

        window->DrawList->AddText(NULL, 0.0f, text_pos + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255), preview_value, NULL, 0.0f, &clip_rect);
        window->DrawList->AddText(NULL, 0.0f, text_pos + ImVec2(1, 0), IM_COL32(0, 0, 0, 255), preview_value, NULL, 0.0f, &clip_rect);
        window->DrawList->AddText(NULL, 0.0f, text_pos + ImVec2(0, -1), IM_COL32(0, 0, 0, 255), preview_value, NULL, 0.0f, &clip_rect);
        window->DrawList->AddText(NULL, 0.0f, text_pos + ImVec2(0, 1), IM_COL32(0, 0, 0, 255), preview_value, NULL, 0.0f, &clip_rect);
        window->DrawList->AddText(NULL, 0.0f, text_pos, IM_COL32(255, 255, 255, 255), preview_value, NULL, 0.0f, &clip_rect);
    }

    if (label_size.x > 0)
    {
        ImVec2 label_pos = ImVec2(rect.Min.x, rect.Min.y);
        window->DrawList->AddText(label_pos + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(label_pos + ImVec2(1, 0), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(label_pos + ImVec2(0, -1), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(label_pos + ImVec2(0, 1), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(label_pos, IM_COL32(255, 255, 255, 255), label);
    }

    if (flags & ImGuiComboFlags_CustomPreview)
    {
        g.ComboPreviewData.PreviewRect = ImRect(clickable.Min.x, clickable.Min.y, clickable.Max.x - 20.0f, clickable.Max.y);
    ...
    return BeginComboPopup(popup_id, clickable, flags);
}
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, IM_COL32(40, 40, 40, 255));
    ...
    if (ret)
    {
        ...
        window->DrawList->AddRect(window_pos, window_pos + window_size, IM_COL32(0, 0, 0, 255), 0, 0, 1.0f);
        window->DrawList->AddRect(window_pos + ImVec2(1, 1), window_pos + window_size - ImVec2(1, 1), IM_COL32(60, 60, 60, 255), 0, 0, 1.0f);
        window->DrawList->AddRectFilled(window_pos + ImVec2(2, 2), window_pos + window_size - ImVec2(2, 2), IM_COL32(40, 40, 40, 255), 0, 0);
    }
...
void ImGui::EndCombo()
{
    ...
    if (g.CurrentWindow->Flags & ImGuiWindowFlags_Popup)
    {
        EndPopup();
        g.BeginComboDepth--;
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth(); // useless since we hardcode it anyways

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float slider_width = 267.f;

    ImVec2 pos = window->DC.CursorPos;

    ImRect frame_bb(ImVec2(pos.x - 1, pos.y) + ImVec2(0, 10.f), window->DC.CursorPos + ImVec2(slider_width, 25));
    ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.f, 10.f));


    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(total_bb, style.FramePadding.y + 30.f);
    if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;
    ...
    ImU32 outline_col = IM_COL32(0, 0, 0, 255);
    ImU32 inline_col_troll = IM_COL32(40, 40, 40, 255);
    window->DrawList->AddRect(frame_bb.Min + ImVec2(2, 2), frame_bb.Max - ImVec2(2, 2), inline_col_troll, g.Style.FrameRounding, 0, 1.0f);

    window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(1, 1), outline_col, g.Style.FrameRounding, 0, 1.0f);
    ...
    {
        ImU32 outline_col = IM_COL32(menu::accent_color.x * 255 + 30, menu::accent_color.y * 255 + 30, menu::accent_color.z * 255 + 30, menu::accent_color.w * 255 + 30);

        ImU32 __col = GetColorU32(menu::accent_color);
        
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, __col , style.GrabRounding, 0);

        //     IMGUI_API void  AddRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, ImDrawFlags flags = 0);  
        window->DrawList->AddRect(grab_bb.Min, grab_bb.Max, outline_col, style.GrabRounding, 0, 1.0f);
    }
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    {
        float grab_t = ScaleRatioFromValueT<float, int, float>(data_type, *(float*)p_data, *(float*)p_min, *(float*)p_max, false, 0.0f, 0.0f);

        float grab_pos = ImLerp(frame_bb.Min.x, frame_bb.Max.x, grab_t);

        ImVec2 value_pos(frame_bb.Max.x - (value_buf_end - value_buf) * g.FontSize * 0.5f - 3.0f, frame_bb.Min.y - g.FontSize - style.ItemInnerSpacing.y + 3.f);

        window->DrawList->AddText(value_pos + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255), value_buf);
        window->DrawList->AddText(value_pos + ImVec2(1, 0), IM_COL32(0, 0, 0, 255), value_buf);
        window->DrawList->AddText(value_pos + ImVec2(0, -1), IM_COL32(0, 0, 0, 255), value_buf);
        window->DrawList->AddText(value_pos + ImVec2(0, 1), IM_COL32(0, 0, 0, 255), value_buf);
    ...
    if (label_size.x > 0.0f)
    {
        ImVec2 label_pos(frame_bb.Min.x + 1, frame_bb.Min.y - label_size.y - style.ItemInnerSpacing.y);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (g.LastItemData.ID != 0)
    {
        ...
        window->DC.CursorPos.x = content_region_max - square_sz - 5.0f;
    }
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        static ImGuiID hovered_button_id = 0;
        static float hover_animation = 0.0f;
        ImGuiID button_id = window->GetID("##ColorButton");

        PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 0));
        SetNextItemWidth(30.0f);

        ImVec2 button_pos = window->DC.CursorPos;
        ImVec2 button_size = ImVec2(30.0f, 13.0f);

        if (ColorButton("##ColorButton", col_v4, flags | ImGuiColorEditFlags_NoTooltip, button_size))
        {
        ...
        if (is_hovered)
            hovered_button_id = button_id;
        else if (hovered_button_id == button_id)
            hovered_button_id = 0;

        float target_hover = (hovered_button_id == button_id) ? 1.0f : 0.0f;
        hover_animation = ImLerp(hover_animation, target_hover, g.IO.DeltaTime * 10.0f);

        ImRect button_bb(button_pos, button_pos + button_size);

        float half_width = (button_bb.Max.x - button_bb.Min.x) / 2.0f;
        float half_height = (button_bb.Max.y - button_bb.Min.y) / 2.0f;

        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(button_bb.Min.x + 1, button_bb.Min.y + 1 + half_height), ImVec2(button_bb.Min.x + 1 + half_width, button_bb.Max.y - 1), 0, 2.0f, ImVec2(0.0f, 0.0f));
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(button_bb.Max.x - 1 - half_width, button_bb.Min.y + 1), ImVec2(button_bb.Max.x - 1, button_bb.Max.y - 1 - half_height), 0, 2.0f, ImVec2(0.0f, 0.0f));

        ImVec4 i_fucking_hate_this(col[0], col[1], col[2], 1.0f);

        window->DrawList->AddRectFilled(ImVec2(button_bb.Min.x + 1, button_bb.Min.y + 1 + half_height), ImVec2(button_bb.Min.x + 1 + half_width, button_bb.Max.y - 1), ColorConvertFloat4ToU32(col_v4), 0.0f);
        window->DrawList->AddRectFilled(ImVec2(button_bb.Max.x - 1 - half_width, button_bb.Min.y + 1), ImVec2(button_bb.Max.x - 1, button_bb.Max.y - 1 - half_height), ColorConvertFloat4ToU32(col_v4), 0.0f);

        window->DrawList->AddRectFilled(ImVec2(button_bb.Min.x + 1, button_bb.Min.y + 1), ImVec2(button_bb.Min.x + 1 + half_width, button_bb.Min.y + 1 + half_height), ColorConvertFloat4ToU32(i_fucking_hate_this), 0.0f);
        window->DrawList->AddRectFilled(ImVec2(button_bb.Max.x - 1 - half_width, button_bb.Max.y - 1 - half_height), ImVec2(button_bb.Max.x - 1, button_bb.Max.y - 1), ColorConvertFloat4ToU32(i_fucking_hate_this), 0.0f);

        window->DrawList->AddRect(button_bb.Min, button_bb.Max, IM_COL32(0, 0, 0, 255), 0.0f, 0, 1.0f);
        ImVec4 inline_color = ImLerp(ImVec4(60.0f / 255.0f, 60.0f / 255.0f, 60.0f / 255.0f, 1.0f), menu::accent_color, hover_animation);
        window->DrawList->AddRect(ImVec2(button_bb.Min.x + 1, button_bb.Min.y + 1), ImVec2(button_bb.Max.x - 1, button_bb.Max.y - 1), ColorConvertFloat4ToU32(inline_color), 0.0f, 0, 1.0f);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            ...
            {
                if (x == 0 && y == 0) continue;
                window->DrawList->AddText(ImVec2(label_pos.x + x, label_pos.y + y), IM_COL32(0, 0, 0, 255), label, label_display_end);
            }
        }
        TextEx(label, label_display_end);
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_picker_size = 150.0f;
    float bars_width = 15.0f;
    ...
    static ImGuiID hovered_picker_id = 0;
    static float hover_animation = 0.0f;
    static ImGuiID hovered_hue_id = 0;
    static float hue_hover_anim = 0.0f;
    static ImGuiID hovered_alpha_id = 0;
    static float alpha_hover_anim = 0.0f;
    static float anim_hue_y = 0.0f;
    static float anim_alpha_x = 0.0f;

    ImGuiID picker_id = window->GetID("sv");
    ImGuiID hue_id = window->GetID("hue");
    ImGuiID alpha_id = window->GetID("alpha");
    ...
    if (is_hovered)
        hovered_picker_id = picker_id;
    else if (hovered_picker_id == picker_id)
        hovered_picker_id = 0;

    float target_hover = (hovered_picker_id == picker_id) ? 1.0f : 0.0f;
    hover_animation = ImLerp(hover_animation, target_hover, g.IO.DeltaTime * 10.0f);
    ...
    hue_hovered = IsItemHovered();
    hue_active = IsItemActive();

    if (hue_hovered)
        hovered_hue_id = hue_id;
    else if (hovered_hue_id == hue_id)
        hovered_hue_id = 0;

    float target_hue_hover = (hovered_hue_id == hue_id) ? 1.0f : 0.0f;
    hue_hover_anim = ImLerp(hue_hover_anim, target_hue_hover, g.IO.DeltaTime * 10.0f);

    if (hue_active && !is_readonly)
    {
    ...
    if (alpha_bar)
    {
        SetCursorScreenPos(ImVec2(picker_pos.x, picker_pos.y + sv_picker_size + style.ItemInnerSpacing.x));
        InvisibleButton("alpha", ImVec2(sv_picker_size, bars_width));
        alpha_hovered = IsItemHovered();
        alpha_active = IsItemActive();

        if (alpha_hovered)
            hovered_alpha_id = alpha_id;
        else if (hovered_alpha_id == alpha_id)
            hovered_alpha_id = 0;

        float target_alpha_hover = (hovered_alpha_id == alpha_id) ? 1.0f : 0.0f;
        alpha_hover_anim = ImLerp(alpha_hover_anim, target_alpha_hover, g.IO.DeltaTime * 10.0f);
    ...
    draw_list->AddRect(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), IM_COL32(30, 30, 30, 255), 0.0f, 0, 1.0f);

    ImVec4 hover_color = menu::accent_color;
    hover_color.w = hover_animation;
    draw_list->AddRect(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ColorConvertFloat4ToU32(hover_color), 0.0f, 0, 1.0f);
    ...
    float target_hue_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
    if (!hue_active)
        anim_hue_y = target_hue_y;
    else
        anim_hue_y = ImLerp(anim_hue_y, target_hue_y, g.IO.DeltaTime * 9.f);

    draw_list->AddRect(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), IM_COL32(30, 30, 30, 255), 0.0f, 0, 1.0f);

    ImVec4 hue_hover_col = menu::accent_color;
    hue_hover_col.w = hue_hover_anim;
    draw_list->AddRect(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), ColorConvertFloat4ToU32(hue_hover_col), 0.0f, 0, 1.0f);

    ImVec4 line_col = menu::accent_color;
    line_col.w = style.Alpha;
    ImU32 line_col32 = ColorConvertFloat4ToU32(line_col);
    draw_list->AddLine(ImVec2(bar0_pos_x, anim_hue_y), ImVec2(bar0_pos_x + bars_width, anim_hue_y), col_black, 3.0f);
    draw_list->AddLine(ImVec2(bar0_pos_x, anim_hue_y), ImVec2(bar0_pos_x + bars_width, anim_hue_y), line_col32, 1.0f);
    ...
    if (alpha_bar)
    {
        ...
        ImRect bar1_bb(picker_pos.x, picker_pos.y + sv_picker_size + style.ItemInnerSpacing.x, picker_pos.x + sv_picker_size, picker_pos.y + sv_picker_size + style.ItemInnerSpacing.x + bars_width);
        ...
        float target_alpha_x = IM_ROUND(picker_pos.x + alpha * sv_picker_size);
        if (!alpha_active) // we check if they are active so they wont tween when we move the window lol
            anim_alpha_x = target_alpha_x;
        else
            anim_alpha_x = ImLerp(anim_alpha_x, target_alpha_x, g.IO.DeltaTime * 9.f);

        draw_list->AddRect(bar1_bb.Min, bar1_bb.Max, IM_COL32(30, 30, 30, 255), 0.0f, 0, 1.0f);

        ImVec4 alpha_hover_col = menu::accent_color;
        alpha_hover_col.w = alpha_hover_anim;
        draw_list->AddRect(bar1_bb.Min, bar1_bb.Max, ColorConvertFloat4ToU32(alpha_hover_col), 0.0f, 0, 1.0f);

        draw_list->AddLine(ImVec2(anim_alpha_x, bar1_bb.Min.y), ImVec2(anim_alpha_x, bar1_bb.Max.y), col_black, 3.0f);
        draw_list->AddLine(ImVec2(anim_alpha_x, bar1_bb.Min.y), ImVec2(anim_alpha_x, bar1_bb.Max.y), line_col32, 1.0f);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        ImGuiStorage* storage = &window->StateStorage;
        float* tween_value = storage->GetFloatRef(ImHashStr("##sel_tween", 0, id), 0.0f);

        float target = (highlighted || selected) ? 1.0f : 0.0f;
        float speed = 8.0f * g.IO.DeltaTime;
        *tween_value = ImLerp(*tween_value, target, speed);

        if (*tween_value > 0.01f)
        {
            ImVec4 accent_color = menu::accent_color;
            float alpha = *tween_value * 100.0f;
            ImU32 bg_col = IM_COL32(accent_color.x * 255, accent_color.y * 255, accent_color.z * 255, alpha);
    ...
    if (is_visible)
    {
        ImVec2 text_pos = ImVec2(bb.Min.x + (bb.GetWidth() - label_size.x) * 0.5f, pos.y + (size.y - label_size.y) * 0.5f);

        window->DrawList->AddText(text_pos + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(text_pos + ImVec2(1, 0), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(text_pos + ImVec2(0, -1), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(text_pos + ImVec2(0, 1), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(text_pos, IM_COL32(255, 255, 255, 255), label);
    }
